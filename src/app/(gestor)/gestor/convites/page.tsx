...
export default function GestorConvitesPage() {
  ...
  const handleVerAceites = (conviteId: string) => {
    router.push(`/gestor/convites/${conviteId}/aceites`);
  };
        ...
        <Card className="border-0 shadow-soft">
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="heading-2">Gerenciar Convites</h1>
      ...
      {convites.length === 0 ? (
        <Card className="border-0 shadow-soft">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {filtroStatus !== 'todos'
                ? `Não há convites com status "${filtroStatus}" no sistema`
                : 'Ainda não há convites criados no sistema'}
            </p>
          </CardContent>
        </Card>
      ) : (
